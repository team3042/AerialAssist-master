...
import org.team3042.AerialAssist.commands.CameraUpdater;
...
public class CameraSystem extends Subsystem {
    ...
    private AxisCamera camera = AxisCamera.getInstance("10.30.42.11");
    ...
    private boolean hotness = true; // Store the last calculated hotness status
    ...
    private void commonCameraConfig() {
        camera.writeColorLevel(50);
        camera.writeWhiteBalance(AxisCamera.WhiteBalanceT.fixedIndoor);
    }

    public void autonCameraConfig() {
        commonCameraConfig();
        camera.writeBrightness(0);
    }

    public void teleopCameraConfig() {
        commonCameraConfig();
        camera.writeBrightness(50);
    }

    private boolean scoreImage(ColorImage image) throws NIVisionException {
        TargetReport target = new TargetReport();
        int verticalTargets[] = new int[MAX_PARTICLES];
        int horizontalTargets[] = new int[MAX_PARTICLES];
        int verticalTargetCount, horizontalTargetCount;
        BinaryImage thresholdImage = image.thresholdHSV(100, 140, 230, 255, 83, 183);   // keep only green objects
        ...
        if (filteredImage.getNumberParticles() > 0) {
            ...
            System.out.println("Particle count was zero!");
        }
         ...
         * memory to accumulate over each pass of this loop.
         */
        filteredImage.free();
        thresholdImage.free();
        image.free();
